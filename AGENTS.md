...
lsr/__main__.py → lsr/main.py:main()
  ...
  ├─ lsr/repo.py        GitRepo (auto-commits, dirty tracking, .lsrignore)
...
- **Dependency injection / state**: `InputOutput` is injected everywhere;
  ...
  Coder switches.
- **Git integration**: `GitRepo` (`lsr/repo.py`) auto-commits with author
  attribution, honors `.lsrignore`. Prefer reusing it over shelling out to git.
...
- **Not yet covered (integration)**: `Coder` run loop, slash-command dispatch
  (`Commands`), `LatexCompiler.compile` (needs pdflatex), `GitRepo`, `RepoMap`.
  These need LLM/git mocking — extend with `Coder.create(...)` + mocked
