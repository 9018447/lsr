...
VERSION_CHECK_FNAME = Path.home() / ".lsr" / "caches" / "versioncheck"


def install_from_main_branch(io):
    """
    Install the latest version of lsr from the main branch of the GitHub repository.
    """

    return utils.check_pip_install_extra(
        io,
        None,
        "Install the development version of lsr from the main branch?",
        ["git+https://github.com/your-username/lsr.git"],
        self_update=True,
    )
...
def install_upgrade(io, latest_version=None):
    """
    Install the latest version of lsr from PyPI.
    """

    if latest_version:
        new_ver_text = f"Newer lsr version v{latest_version} is available."
    else:
        new_ver_text = "Install latest version of lsr?"
...
"""
    ...
    if success:
        io.tool_output("Re-run lsr to use new version.")
        sys.exit()
...
def check_version(io, just_check=False, verbose=False):
    ...
    try:
        response = requests.get("https://pypi.org/pypi/lsr/json")
        data = response.json()
        latest_version = data["info"]["version"]
        current_version = lsr.__version__
