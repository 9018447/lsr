...
class Coder:
    ...
    repo = None
    last_lsr_commit_hash = None
    lsr_edited_files = None
    ...
    def __init__(
        ...
        if lsr_commit_hashes:
            self.lsr_commit_hashes = lsr_commit_hashes
        else:
            self.lsr_commit_hashes = set()
        ...
        for fname in fnames:
            ...
            if self.repo and self.repo.ignored_file(fname):
                self.io.tool_warning(f"Skipping {fname} that matches lsrignore spec.")
                continue
        ...
        while message:
            ...
            if getattr(self, "edit_format", None) == "plan" and self.partial_response_content:
                ...
                if "## Plan:" in content or "Type `/code` to execute" in content:
                    ...
                    try:
                        from lsr.plan_manager import save_plan
    ...
    def show_auto_commit_outcome(self, res):
        commit_hash, commit_message = res
        self.last_lsr_commit_hash = commit_hash
        self.lsr_commit_hashes.add(commit_hash)
        self.last_lsr_commit_message = commit_message
        if self.show_diffs:
