...
lsr_user_agent = f"LSR/{__version__} +{urls.website}"
...
def install_playwright(io):
    ...
    pip_cmd = utils.get_pip_install(["lsr[playwright]"])
...
class Scraper:
    ...
    def scrape_with_httpx(self, url):
        ...
        headers = {"User-Agent": f"Mozilla./5.0 ({lsr_user_agent})"}
        try:
            with httpx.Client(
